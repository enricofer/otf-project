...
def generate_legend(layers, project):
    ...
    """
    QgsMessageLog.logMessage('progress a')
...
def is_file_path(uri):
    ...
    :rtype: bool
    """
    QgsMessageLog.logMessage('is_file_path: ')
    try:
...
def is_tile_path(uri):
    ...
    :rtype: bool
    """
    QgsMessageLog.logMessage('is_tile_path: ')
    try:
...
def is_pg_path(uri):
    QgsMessageLog.logMessage('is_pg_path: ')
    sanitized_uri = unquote(uri)
    query_params = parse_qs(sanitized_uri)
    QgsMessageLog.logMessage('pg params: ' + str(query_params))
    query_params_keys = [k.lower() for k in query_params.keys()]
    QgsMessageLog.logMessage('pg params keys: ' + str(query_params_keys))
